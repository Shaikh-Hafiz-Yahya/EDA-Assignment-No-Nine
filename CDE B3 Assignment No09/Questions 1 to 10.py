...
users_last_10 = joined_pr.sort_values('date', ascending=False)['user'].head(10)
...
Now that we have identified two potential contacts in the projects, we need to find the person who was most involved in the
project in recent times. That person is most likely to answer our questions. For each calendar year, we are interested in 
understanding the number of pull requests the authors submitted. This will give us a high-level image of their contribution
trend to the project."""
...
# Plot the results
counts_wide.plot(kind='bar', figsize=(10,5), title='Pull Requests per Year by Developer')


""" 
10. Visualizing the contributions of each developer
As mentioned before, it is important to make a distinction between the global expertise and contribution levels and the 
contribution levels at a more granular level (file, submodule, etc.) In our case, we want to see which of our two developers
of interest have the most experience with the code in a given file. We will measure experience by the number of pull requests
submitted that affect that file and how recent those pull requests were submitted."""
...
# Plot the results
by_file_wide.plot(kind='bar', figsize=(10,5), title='Contributions by Year for File: Calculate.scala')
